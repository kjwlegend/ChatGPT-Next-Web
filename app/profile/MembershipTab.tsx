...
const MembershipTab = () => {
  ...
  const [selectedMembershipType, setSelectedMembershipType] = useState("normal" as member_type);
  const [paymentMethod, setPaymentMethod] = useState("1"); // 默认选择小光币
  
  // 我需要基于selectedMembershipType 生成对应的中文 selectedMembershipName, 
  const [selectedMembershipName, setSelectedMembershipName] = useState("普通会员");
  useEffect(() => {
	...
	return (
      ...
      >
        <div className={styles['modal-content-container']}>
          ...
          />
          <div>
		  <p className={styles['service']}>购买的服务：{selectedMembershipName}</p>
			<p className={styles['balance']}>当前余额：{user.xgb_balance}</p>
			<p className={styles['deduction']}>扣款金额：{getPrice(selectedMembershipType)}</p>
			<p className={styles['remaining']}>剩余金额：{user.xgb_balance - getPrice(selectedMembershipType)}</p>
			<Radio.Group onChange={handlePaymentMethodChange} value={paymentMethod} className={styles['payment-method']}>
			<Radio value="1">小光币</Radio>
			<Radio value="2" disabled>微信</Radio>
			<Radio value="3" disabled>支付宝</Radio>
			</Radio.Group>
          </div>
        </div>
      </Modal>
